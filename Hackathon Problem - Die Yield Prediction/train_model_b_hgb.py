"""
================================================================================
Model B: HistGradientBoosting (Die + Spatial + Block Statistics from 2,000 Readings)
...
def main():
    ...
    print("=" * 80)
    print("  Model B: HistGradientBoosting (Die + Spatial + Block Statistics)")
    print("=" * 80)
    ...
    X_train_full = train_eligible[all_features_b].values
    ...
    X_test = test_eligible[all_features_b].values
    y_test = test_eligible["label"].values.astype(int)

    # 6. Train Balanced HistGradientBoosting Classifier (Model B)
    print("\nTraining Balanced HistGradientBoosting Classifier (Model B)...")
    ...
    # 7. Threshold Optimization on Validation Fold
    print("\nOptimizing decision threshold on validation fold...")
    val_probs_b = hgb_b.predict_proba(X_val)[:, 1]
    ...
    print(f"Optimal Threshold for Model B (HistGradientBoosting): {optimal_threshold_b:.4f} (Validation Fail F1: {best_val_f1_b:.4f})")

    # 8. Evaluate on Test Set (Strictly Eligible Dies)
    test_probs_b = hgb_b.predict_proba(X_test)[:, 1]
    ...
    print("\n" + "=" * 80)
    print("          MODEL B (HIST GRADIENT BOOSTING) TEST RESULTS         ")
    print("=" * 80)
    ...
    perm_res = permutation_importance(hgb_b, X_val[val_sample_idx], y_val[val_sample_idx], n_repeats=3, random_state=42, n_jobs=-1)
    
    feat_series_b = pd.Series(perm_res.importances_mean, index=all_features_b)
    top25_b = feat_series_b.sort_values(ascending=False).head(25)
    ...
    plt.title("Model B: Top 25 Permutation Feature Importances (HistGradientBoosting)\n[Green: Block Signals | Red: Spatial Context | Blue: Parametric]", fontsize=12)
    plt.xlabel("Mean Accuracy Decrease on Permutation")
    plt.tight_layout()
    fi_path_b = "reports/model_b_hgb_feature_importance.png"
    plt.savefig(fi_path_b, dpi=200)
    plt.close()
    print(f"  Saved: {fi_path_b}")

    # 10. Export Model B Predictions
    print("\nExporting Model B (HistGradientBoosting) submission predictions...")
    all_test_block = extract_block_features_parallel(test_df["block_readings"].values)
    for idx, col in enumerate(block_col_names):
        test_df[col] = all_test_block[:, idx]

    all_test_X_b = test_df[all_features_b].values
    all_test_probs_b = hgb_b.predict_proba(all_test_X_b)[:, 1]
    ...
    sub_path_b = "predictions_model_b_hgb.csv"
