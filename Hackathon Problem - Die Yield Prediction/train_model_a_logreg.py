"""
================================================================================
Model A: Logistic Regression (500 Parametric Features + Spatial Context)
...
def main():
    ...
    print("=" * 80)
    print("  Model A: Logistic Regression (500 Parametric Features + Spatial Context)")
    print("=" * 80)
    ...
    X_train_raw = train_eligible[all_features].values
    ...
    X_test_raw = test_eligible[all_features].values
    y_test = test_eligible["label"].values.astype(int)

    # 4. Standardize Features (Essential for Logistic Regression with L2 regularization)
    print("\nStandardizing 507 features with StandardScaler...")
    ...
    print("\nTraining Balanced Logistic Regression Classifier...")
    ...
    # 6. Threshold Optimization on Validation Fold
    ...
    print(f"Optimal Threshold for Logistic Regression: {optimal_threshold:.4f} (Validation Fail F1: {best_val_f1:.4f})")

    # 7. Evaluate on Test Set (Strictly Eligible Dies)
    ...
    print("\n" + "=" * 80)
    print("            MODEL A (LOGISTIC REGRESSION) TEST RESULTS           ")
    print("=" * 80)
    ...
    print("\nGenerating Logistic Regression Interpretability Artifacts...")
    os.makedirs("reports", exist_ok=True)

    coefs = np.abs(logreg.coef_[0])
    coef_series = pd.Series(coefs, index=all_features)
    top25 = coef_series.sort_values(ascending=False).head(25)

    plt.figure(figsize=(10, 8))
    colors = ["#d9534f" if feat in spatial_features else "#337ab7" for feat in top25.index[::-1]]
    top25[::-1].plot(kind="barh", color=colors)
    plt.title("Model A: Top 25 Absolute Coefficients (Logistic Regression)\n[Red: Spatial Context | Blue: Parametric]", fontsize=12)
    plt.xlabel("|Standardized Coefficient|")
    plt.tight_layout()
    fi_path = "reports/model_a_logreg_feature_importance.png"
    plt.savefig(fi_path, dpi=200)
    plt.close()
    print(f"  Saved: {fi_path}")

    # 9. Export Submission Predictions
    print("\nExporting Model A (Logistic Regression) submission predictions...")
    all_test_X_raw = test_df[all_features].values
    all_test_X_scaled = scaler.transform(all_test_X_raw)
    all_test_probs = logreg.predict_proba(all_test_X_scaled)[:, 1]
    ...
    sub_path = "predictions_model_a_logreg.csv"
