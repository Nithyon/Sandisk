"""
================================================================================
Model B: Extra Trees Classifier (Die + Spatial + Block Statistics from 2,000 Readings)
...
def main():
    ...
    print("=" * 80)
    print("  Model B: Extra Trees Classifier (Die + Spatial + Block Statistics)")
    print("=" * 80)
    ...
    # 6. Train Balanced Extra Trees for Model B
    print("\nTraining Balanced Extra Trees Classifier (Model B)...")
    ...
    val_probs_b = et_b.predict_proba(X_val)[:, 1]
    ...
    print(f"Optimal Threshold for Model B (Extra Trees): {optimal_threshold_b:.4f} (Validation Fail F1: {best_val_f1_b:.4f})")
    ...
    test_probs_b = et_b.predict_proba(X_test)[:, 1]
    ...
    print("\n" + "=" * 80)
    print("              MODEL B (EXTRA TREES) TEST RESULTS             ")
    print("=" * 80)
    ...
    print("\nGenerating Model B (Extra Trees) Interpretability Artifacts...")
    os.makedirs("reports", exist_ok=True)

    importances_b = et_b.feature_importances_
    feat_series_b = pd.Series(importances_b, index=all_features_b)
    ...
    all_test_probs_b = et_b.predict_proba(all_test_X_b)[:, 1]
    ...
    sub_path_b = "predictions_model_b_extratrees.csv"
