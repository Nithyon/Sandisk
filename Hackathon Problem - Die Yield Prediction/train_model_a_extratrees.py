"""
================================================================================
Model A: Extra Trees Classifier (500 Parametric + Spatial Context)
...
def main():
    ...
    print("=" * 75)
    print("  Model A: Extra Trees Classifier (500 Parametric Features + Spatial Context)")
    print("=" * 75)
    ...
    print(f"Training split: {len(X_tr):,} dies | Validation split: {len(X_val):,} dies")

    # 4. Train Extra Trees Classifier
    print("\nTraining Balanced Extra Trees Classifier...")
    ...
    val_probs = et.predict_proba(X_val)[:, 1]
    ...
    test_probs = et.predict_proba(X_test)[:, 1]
    ...
    print("\n" + "=" * 75)
    print("              MODEL A (EXTRA TREES) TEST RESULTS             ")
    print("=" * 75)
    ...
    print("\nGenerating Extra Trees Interpretability Artifacts...")
    ...
    plt.title("Model A: Top 25 Feature Importances (Extra Trees)\n[Red: Spatial Context | Blue: Parametric]", fontsize=12)
    plt.xlabel("Gini Feature Importance")
    plt.tight_layout()
    fi_path = "reports/model_a_extratrees_feature_importance.png"
    plt.savefig(fi_path, dpi=200)
    plt.close()
    print(f"  Saved: {fi_path}")

    # 8. Export Submission Predictions
    print("\nExporting Model A (Extra Trees) submission predictions...")
    all_test_X = test_df[all_features].values
    all_test_probs = et.predict_proba(all_test_X)[:, 1]
    ...
    sub_path = "predictions_model_a_extratrees.csv"
